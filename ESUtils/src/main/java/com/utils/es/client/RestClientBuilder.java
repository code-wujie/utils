...
public final class RestClientBuilder {
    ...
    public RestClient build() {
        ...
        AbstractHttpAsyncClient httpClient = createHttpClient();
        RestClient restClient = new RestClient(httpClient, maxRetryTimeout, defaultHeaders, hosts, pathPrefix, failureListener);
        httpClient.start();
        return restClient;
    }

    private AbstractHttpAsyncClient createHttpClient() {
        //设置参数
        IOReactorConfig config = new IOReactorConfig();
        if (requestConfigCallback != null) {
        	config = requestConfigCallback.customizeRequestConfig(config);
        }
        //设置回调的函数
        AbstractHttpAsyncClient asyncClient = null;
		try {
			asyncClient = new DefaultHttpAsyncClient(config);
		} catch (IOReactorException e) {
			e.printStackTrace();
		}
        if (httpClientConfigCallback != null) {
        	asyncClient = httpClientConfigCallback.customizeHttpClient(asyncClient);
        }
        //返回数据
        return asyncClient;
    }
    ...
    /**
     * Callback used the default {@link } being set to the {@link }
     * @see #
     */
    public interface RequestConfigCallback {
        /**
         * Allows to customize the {@link } that will be used with each request.
         ...
         */
    	IOReactorConfig customizeRequestConfig(IOReactorConfig ioReactorConfig);
    }

    /**
     * Callback used to customize the {@link } instance used by a {@link RestClient} instance.
     * Allows to customize default {@link } being set to the client and any parameter that
     * can be set through {@link }
     */
    public interface HttpClientConfigCallback {
        /**
         * Allows to customize the {@link } being created and used by the {@link RestClient}.
         * Commonly used to customize the default {@link org.apache.http.client.CredentialsProvider} for authentication
         * or the {@link } for communication through ssl without losing any other useful default
         * value that the {@link RestClientBuilder} internally sets, like connection pooling.
         */
    	AbstractHttpAsyncClient customizeHttpClient(AbstractHttpAsyncClient httpClient);
    }
}
